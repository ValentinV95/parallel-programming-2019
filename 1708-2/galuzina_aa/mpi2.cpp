...
int Consumer(int *buffer, int buffer_size, int rank_proc, int* resurce) {
	...
	if (size == 1 || rank_proc == 0) { // если 0 процес выполнет функцию или кол-во проц = 1
		...
		if (rank == 0) {
			...
			MPI_Send(&temp_resurs, 1, MPI_INT, rank_proc, 0, MPI_COMM_WORLD); //отправл€ет ресурс процессору который его вызвал
		}
		else {
			if (rank == rank_proc) {
				int temp_resurs;
				MPI_Recv(&temp_resurs, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);//принимает данные от производител€ на 0
				*resurce = temp_resurs; // отправленный ресурс делает прин€тым 
			}
		}
	}
...
int Producer(int *buffer, int buffer_size, int rank_proc, int resurce) {
	...
	if (size == 1 || rank_proc == 0) {
		...
		if (rank == 0) { //если функцию вызвал 0 процессор, то принимает
			...
			MPI_Recv(&resurce_for_bufer, 1, MPI_INT, rank_proc, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); //прием сообщени€ процессора вызывающего функцию производителей
	...
	MPI_Barrier(MPI_COMM_WORLD); //Ѕлокирует звон€щего до тех пор, пока все члены группы не вызовут его; вызов возвращаетс€ в любом процессе только после того, как все члены группы вошли в вызов.
	return 0;
}
int main(int argc, char** argv)
{
	...
	if (size == 1) {
		...
		for (int i = 0; i < kol_resursov; i++) {
			Consumer(buffer, kol_elem_in_buffer, rank, &resurce_consume[i]); //отправл€ю данные в массиве 
			et2 = MPI_Wtime();
			cout << resurce_consume[i] << "\n" << "Time_Single" << st2-et2 << std::fixed << " sec \n";;
		}
	}
	else { // многопроцесорна€ 
		st1 = MPI_Wtime();
		for (int i = 0; i < kol_resursov; i++) {
			Producer(buffer, kol_elem_in_buffer, 1, i); //отправл€ем по кол-ву ресурсов отправл€ем кол-во эл в буфере 
		}
		std::vector<int> resurce_consume1(kol_resursov, -1);
		int *resurce_consume;
		resurce_consume = Create_dinamic_massiv_from_vector(resurce_consume1);
		for (int i = 0; i < kol_resursov; i++) {
			Consumer(buffer, kol_elem_in_buffer, 1, &resurce_consume[i]); //отправл€ю -1 (массив)
		}
