...
namespace functions
{
		...
		std::vector<double> points)
	{
		...
		{
			auto temp = (double)(std::fabs(points[i] - points[i - 1])) /
	...
	{
		MPI_Status status;
		MPI_Recv(max_num, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD,
			&status);
	}

	std::vector<double> Impl::get_result(double a, double b, double r, double min_error_bound,
		double(*func)(double))
	{
		if (b <= a)
		{
