               ...
               std::vector<double> points)
{
	...
	{
		auto temp = static_cast<double>(fabs(points[i] - points[i - 1])) /
                          ...
                          double f_x_left, double f_x_right, double max)
{
	...
	{
		return 2 * (delta) - 4 *
			(static_cast<double>(f_x_right) / static_cast<double>(max));
	}

	if (val2 == x_right)
	{
		return 2 * (delta) - 4 *
			(static_cast<double>(f_x_left) / static_cast<double>(max));
	}

	return x_right - x_lleft +
		static_cast<double>(std::pow(f_x_right - f_x_left, 2)) /
		static_cast<double>(std::pow(max, 2) * delta) - 2 *
		(static_cast<double>(f_x_right + f_x_left) / static_cast<double>(max));
}
                 ...
                 double f_x_r, double max, int max_num, int size)
{
	...
	{
		return static_cast<double>(x_right + x_left) / 2.0;
	}

	return (static_cast<double>(x_right + x_left) / 2.0) -
		(static_cast<double>(f_x_r - f_x_left) / (2.0 * max));
}

void InitializePoints(double a, double b, int size, std::vector<double>& points)
{
	auto range = static_cast<double>(b - a) / static_cast<double>(size + 1);
		  ...
		  double& M, std::vector<double>& points)
{
	...
	for (auto i = 0; i < static_cast<int>(f_points.size()); i++)
	{
...
void PrepareMaxIntervalsVector(int size, std::vector<int>& m_intervals,
							   std::vector<double>& d_vec, int iterationNumber)
{
	double minElem;
							   ...
							   double min_error_bound, double (*func)(double))
{
	...
	while (true)
	{
		...
		if (curr_error <= min_error_bound) break;
		...
		std::vector<double> f_range(length);
