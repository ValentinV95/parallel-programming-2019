...
TEST(Global_Search_MPI, Fail_Wrong_Length)
{
	int rank;
	MPI_Comm_rank(MPI_COMM_WORLD, &rank);
	std::vector<double> actual(2);
...
TEST(Global_Search_MPI, Transform_Function1)
{
	int rank;
	MPI_Comm_rank(MPI_COMM_WORLD, &rank);
...
TEST(Global_Search_MPI, Transform_Function2)
{
	int rank;
	MPI_Comm_rank(MPI_COMM_WORLD, &rank);
...
TEST(Global_Search_MPI, Transform_Function3)
{
	int rank;
	MPI_Comm_rank(MPI_COMM_WORLD, &rank);
