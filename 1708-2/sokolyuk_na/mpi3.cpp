...
int main(int argc, char* argv[]) {
	...
	TimeStartWork2 = MPI_Wtime();//time
	std::vector <Pixel> answer = solveParallel(a, rows, cols);
	TimeEndWork2 = MPI_Wtime();//time
	if (ProcRank == 0) {
		TimeStartWork = MPI_Wtime();//time
		std::vector <Pixel> seqAnswer = solveSequential(a, rows, cols);
		TimeEndWork = MPI_Wtime();//time
		for (int i = 0; i < rows; ++i) {
			for (int j = 0; j < cols; ++j) {
		...
		if (ProcRank == 0) {
			if (test = true) {
				cout << "\nDate In Paralel = Date In Single"  << endl;
				cout << "\nTime In Solo = " << TimeEndWork - TimeStartWork << " sec" << endl;
				cout << "\nTime In Paralel = " << TimeEndWork2 - TimeStartWork2 << " sec" << endl;
			}
			else
				cout << "\nERRORE" << endl;
		}
