...
class Multiplicator {
 ...
 public:
    Multiplicator(const Matrix* _a, const Matrix* _b,
                  vector<vector<complex<double> > >* _vals,
                  vector<vector<int> >* _rows,
                  int* _pointer, int _n): a(_a), b(_b),
                  vals(_vals), rows(_rows), pointer(_pointer), N(_n) {}
...
void TBBMult(const Matrix& a, const Matrix& b, Matrix* c, int N, int num_threads) {
  c->pointer = new int[N + 1];
  for (int i = 0; i <= N; i++) {
    c->pointer[i] = 0;
  }
  ...
  tbb::parallel_for(tbb::blocked_range<int>(0, N),
    Multiplicator(&a, &b, &vals, &rows, c->pointer, N));
...
int main(int argc, char** argv) {
  ...
  s_time = tbb::tick_count::now();
  Transp(&ccs_a, N);
  Matrix ccs_c;
  linMult(ccs_a, ccs_b, &ccs_c, N);
  double sereal_time = (tbb::tick_count::now() - s_time).seconds();
  ...
  Matrix tbb_c;
  p_time = tbb::tick_count::now();
  TBBMult(ccs_a, ccs_b, &tbb_c, N, num_threads);
  double parallel_time = (tbb::tick_count::now() - p_time).seconds();
  std::cout << "Parallel time: " << parallel_time << std::endl;
  std::cout << "Compare parallel and linear version " << compareCCS(&ccs_c, &tbb_c) << std::endl;
