...
int main(int argc, char ** argv)
{
	...
	//предположим, что число процессов равно степени 2
	...
	for (partner = size / 2; partner > 0; partner = partner >> 1)
	{
		...
		{
			//слияние
			if ((rank / partner) % 2 == 0) //хранение маленьких элем
			{
			...
			else  //хранение более крупных элем
			{
	...
	int * sizeArr = 0, *fullArr = 0, *displacement = 0;
	if (rank == 0)
	{
	...
	if (rank == 0)
	{
		...
		//сканирование на sizeArr для определения смещ каждого местополож данных.
		for (i = 1; i < size; i++)
		{
...
//выбор середины в arr[lo], arr[hi], arr[(lo+hi)/2]
