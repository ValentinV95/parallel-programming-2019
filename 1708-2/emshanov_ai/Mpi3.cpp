...
int main(int argc, char** argv) {
    ...
    double timeStart, endTime, timeStart2, endTime2;
    timeStart = MPI_Wtime();
    BatcherSort(&arrP);
    endTime = MPI_Wtime();
    
    if (myrank == 0) {
        timeStart2 = MPI_Wtime();
        ...
        endTime2= MPI_Wtime();
        std::cout << "time : " << endTime - timeStart << " on proc with BatcherSort " << std::endl;
        std::cout << "time : " << endTime2- timeStart2 << " on proc without BatcherSort " << std::endl;
        if (AreEq == true) {
            std::cout << "Array Equal";
        }
        else {
            std::cout << "Array not Equal";
        }
    }
    MPI_Finalize();
    return 0;
